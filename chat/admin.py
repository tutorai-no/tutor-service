...
class ChatAdmin(admin.ModelAdmin):
    list_display = [
        'title', 'user', 'chat_type', 'status', 'course', 
        'message_count', 'is_pinned', 'last_active_at', 'created_at'
    ]
    list_filter = [
        'chat_type', 'status', 'is_pinned', 'is_favorite',
        'use_course_context', 'use_document_context', 'created_at'
    ]
    search_fields = ['title', 'user__username', 'course__name']
    readonly_fields = [
        'id', 'message_count', 'total_tokens_used', 'average_response_time_ms',
        'last_active_at', 'context_summary', 'created_at', 'updated_at'
    ]
    ...
    fieldsets = (
        ('Basic Information', {
            'fields': ('user', 'title', 'chat_type', 'status')
        }),
        ('Context', {
            'fields': ('course', 'section', 'current_session')
        }),
        ('AI Configuration', {
            'fields': (
        ...
        ('Statistics', {
            'fields': (
                'message_count', 'total_tokens_used', 'average_response_time_ms'
            )
        }),
        ('User Preferences', {
...
class ChatMessageAdmin(admin.ModelAdmin):
    list_display = [
        'message_preview', 'chat', 'role', 'message_type', 
        'is_helpful', 'user_rating', 'created_at'
    ]
    list_filter = [
        'role', 'message_type', 'is_helpful', 'user_rating',
        'is_context_relevant', 'is_deleted', 'created_at'
    ]
    ...
    readonly_fields = [
        ...
        'context_used', 'thread_depth', 'created_at', 'updated_at'
    ]
    ...
    fieldsets = (
        ...
        ('Threading', {
            'fields': ('parent_message', 'thread_depth')
        }),
        ('AI Information', {
            'fields': (
                'ai_model_used', 'temperature_used', 'token_count',
                'prompt_tokens', 'completion_tokens', 'processing_time_ms'
            )
        }),
        ('Context & References', {
            'fields': (
                'context_used', 'referenced_documents', 'referenced_assessments'
            )
        }),
        ('User Feedback', {
        ...
        ('Context Settings', {
            'fields': ('is_context_relevant', 'context_weight')
        }),
        ('Status', {
            'fields': ('is_edited', 'is_deleted', 'edit_history')
        }),
        ('Attachments', {
            'fields': ('attachments',)
        }),
...
class ChatContextAdmin(admin.ModelAdmin):
    list_display = [
        'title', 'chat', 'context_type', 'relevance_score',
        'is_active', 'usage_count', 'last_used_at'
    ]
    list_filter = [
        'context_type', 'is_active', 'source_type', 'created_at'
    ]
    ...
    fieldsets = (
        ...
        ('Relevance', {
            'fields': ('relevance_score', 'is_active')
        }),
        ('Source Information', {
        ...
        ('Relationships', {
            'fields': ('related_documents', 'related_assessments')
        }),
        ('Timestamps', {
            'fields': ('created_at', 'updated_at'),
            'classes': ('collapse',)
...
class TutoringSessionAdmin(admin.ModelAdmin):
    list_display = [
        'title', 'user', 'session_type', 'status', 'course',
        'duration_minutes', 'user_satisfaction', 'created_at'
    ]
    list_filter = [
        'session_type', 'status', 'difficulty_level', 'preferred_learning_style',
        'user_satisfaction', 'learning_effectiveness', 'created_at'
    ]
    search_fields = ['title', 'description', 'user__username', 'course__name']
    readonly_fields = [
        'id', 'duration_minutes', 'is_active', 'total_interactions',
        'total_tokens_used', 'average_response_time_ms', 'created_at', 'updated_at'
    ]
    ...
    def start_sessions(self, request, queryset):
        ...
        self.message_user(
            request,
            f"Successfully started {updated} tutoring session(s)."
        )
    ...
    def complete_sessions(self, request, queryset):
        ...
        self.message_user(
            request,
            f"Successfully completed {updated} tutoring session(s)."
        )
    ...
    fieldsets = (
        ...
        ('Learning Objectives', {
            'fields': (
                'learning_objectives', 'topics_covered', 'skills_practiced'
            )
        }),
        ('Configuration', {
            'fields': (
                'preferred_learning_style', 'difficulty_level',
                'tutor_persona', 'teaching_approach'
            )
        }),
        ('Timing', {
            'fields': (
        ...
        ('Outcomes', {
            'fields': (
                'objectives_achieved', 'concepts_mastered', 'areas_for_improvement'
            )
        }),
        ('Assessment', {
            'fields': (
                'user_satisfaction', 'learning_effectiveness', 'session_notes'
            )
        }),
        ('Performance Metrics', {
            'fields': (
        ...
        ('Follow-up', {
            'fields': (
                'next_session_topics', 'recommended_resources', 'homework_assignments'
            )
        }),
        ('Status', {
            'fields': ('is_active',)
...
class ChatAnalyticsAdmin(admin.ModelAdmin):
    list_display = [
        'user', 'analytics_type', 'period_start', 'period_end',
        'total_chats', 'total_messages', 'active_chat_days'
    ]
    list_filter = [
        'analytics_type', 'period_start', 'period_end', 'created_at'
    ]
    ...
    fieldsets = (
        ...
        ('Engagement Metrics', {
            'fields': (
                'active_chat_days', 'average_messages_per_chat',
