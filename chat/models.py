...
class Chat(models.Model):
    ...
    ai_model = models.CharField(
        max_length=100, default='gpt-4',
        help_text="AI model used for this chat"
    )
    ...
    context_window_messages = models.PositiveIntegerField(
        default=10,
        help_text="Number of messages to keep in context"
    )
    use_course_context = models.BooleanField(
        default=True,
        help_text="Include course materials in context"
    )
    use_document_context = models.BooleanField(
        default=True,
        help_text="Include document content in context"
    )
    use_assessment_context = models.BooleanField(
        default=True,
        help_text="Include assessment history in context"
    )
    ...
    def get_context_messages(self):
        ...
        ).order_by('-created_at')[:self.context_window_messages]
...
class ChatMessage(models.Model):
    ...
    context_used = models.TextField(
        blank=True, null=True,
        help_text="Context information used for AI response"
    )
    ...
    is_context_relevant = models.BooleanField(
        default=True,
        help_text="Whether this message should be included in context"
    )
...
class ChatContext(models.Model):
    ...
    content_hash = models.CharField(
        max_length=64,
        help_text="Hash of content for deduplication"
    )
    ...
    is_active = models.BooleanField(
        default=True,
        help_text="Whether this context is currently active"
    )
...
class TutoringSession(models.Model):
    ...
    learning_objectives = models.JSONField(
        default=list,
        help_text="List of learning objectives for this session"
    )
    topics_covered = models.JSONField(
        default=list,
        help_text="Topics covered in this session"
    )
    skills_practiced = models.JSONField(
        default=list,
        help_text="Skills practiced in this session"
    )
    ...
    tutor_persona = models.CharField(
        max_length=100,
        default='helpful_teacher',
        help_text="AI tutor persona/style"
    )
    ...
    objectives_achieved = models.JSONField(
        default=list,
        help_text="Learning objectives that were achieved"
    )
    concepts_mastered = models.JSONField(
        default=list,
        help_text="Concepts that were mastered"
    )
    ...
    next_session_topics = models.JSONField(
        default=list,
        help_text="Suggested topics for next session"
    )
    recommended_resources = models.JSONField(
        default=list,
        help_text="Recommended study resources"
    )
    homework_assignments = models.JSONField(
        default=list,
        help_text="Homework or practice assignments"
    )
...
class ChatAnalytics(models.Model):
    ...
    def __str__(self):
        return f"Analytics: {self.get_analytics_type_display()} for {self.user.username}"