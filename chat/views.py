...
from .serializers import (
    ...
    ContextRecommendationSerializer,
...
class ChatViewSet(viewsets.ModelViewSet):
        ...
        if serializer.is_valid():
            ...
            if message.role == 'user':
                ...
                response_data = {
                    ...
                    'ai_response': ChatMessageSerializer(ai_response).data if ai_response else None
                }
    ...
    def add_context(self, request, pk=None):
        ...
        if serializer.is_valid():
            ...
            return Response(ChatContextSerializer(context_item).data, status=status.HTTP_201_CREATED)
    ...
    def stats(self, request):
        ...
        total_tokens_used = chats.aggregate(total_tokens=Sum('total_tokens_used'))['total_tokens'] or 0
        
        # Response time statistics
        assistant_messages = messages.filter(role='assistant', processing_time_ms__isnull=False)
        avg_response_time = assistant_messages.aggregate(
            avg_time=Avg('processing_time_ms')
        )['avg_time'] or 0
        
        # Chat type distribution
        by_type = chats.values('chat_type').annotate(count=Count('id')).order_by('-count')
    ...
    def dashboard(self, request):
        ...
        dashboard_data = {
            ...
            'active_sessions': TutoringSessionSummarySerializer(active_sessions, many=True).data,
        }
...
class ChatMessageViewSet(viewsets.ModelViewSet):
    ...
    def reply(self, request, pk=None):
        ...
        serializer = MessageCreateSerializer(
            data=data,
            context={'chat': parent_message.chat, 'request': request}
        )
        
        if serializer.is_valid():
            reply_message = serializer.save()
            return Response(ChatMessageSerializer(reply_message).data, status=status.HTTP_201_CREATED)
...
class TutoringSessionViewSet(viewsets.ModelViewSet):
    ...
    def stats(self, request):
        ...
        avg_satisfaction = rated_sessions.aggregate(
        ...
        avg_effectiveness = effectiveness_sessions.aggregate(
            avg_effectiveness=Avg('learning_effectiveness')
        )['avg_effectiveness'] or 0
        
        # Session type distribution
        by_type = sessions.values('session_type').annotate(count=Count('id')).order_by('-count')
        ...
        stats = {
            ...
            'by_type': list(by_type),
            'recent_sessions': TutoringSessionSummarySerializer(recent_sessions, many=True).data,
...
class ChatAnalyticsViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def insights(self, request):
        ...
        recent_chats = Chat.objects.filter(
            user=user,
            last_active_at__gte=timezone.now() - timezone.timedelta(days=30)
        )
    ...
    def trends(self, request):
        ...
        chat_trends = ChatMessage.objects.filter(
            chat__user=user,
        ...
        # Get session trends
        session_trends = TutoringSession.objects.filter(
            user=user,
            actual_start_time__gte=timezone.now() - timezone.timedelta(days=30)
        ).annotate(
            date=TruncDate('actual_start_time')
        ).values('date').annotate(
