...
class SpacedRepetitionService:
    ...
    def calculate_next_review(
        ...
        if quality_response >= 3:
            ...
            ease_factor = ease_factor + (0.1 - (5 - quality_response) * (0.08 + (5 - quality_response) * 0.02))
        else:
    ...
    def calculate_study_load(flashcards: list) -> dict:
        ...
        due_today = len([f for f in flashcards if f.next_review_date.date() <= now.date()])
        ...
        return {
            ...
            'study_pressure': min(100, (overdue * 2 + due_today) / len(flashcards) * 100) if flashcards else 0,
        }
