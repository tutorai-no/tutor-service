...
class QuizQuestion(BaseModel):
    ...
    answer_options: List[str] = Field(description="Answer options for multiple choice", default=[])
    ...
    cognitive_domain: str = Field(description="Bloom's taxonomy level", default="remember")
    points: int = Field(description="Point value", default=1)
    estimated_time_minutes: float = Field(description="Estimated time to answer", default=1.0)
    ...
    distractor_analysis: Dict[str, str] = Field(description="Analysis of wrong answers", default={})
    prerequisite_knowledge: List[str] = Field(description="Required prior knowledge", default=[])
...
class QuizStructure(BaseModel):
    ...
    question_type_distribution: Dict[str, int] = Field(description="Distribution by question type")
    difficulty_progression: List[str] = Field(description="Difficulty sequence")
    cognitive_distribution: Dict[str, int] = Field(description="Cognitive level distribution")
    ...
    section_organization: List[Dict[str, Any]] = Field(description="Quiz sections", default=[])
...
class QuizGenerationStrategy(BaseModel):
    ...
    engagement_techniques: List[str] = Field(description="Techniques to maintain engagement")
    quality_criteria: List[str] = Field(description="Quality assessment criteria")
    adaptive_elements: List[str] = Field(description="Adaptive quiz features", default=[])
...
class QuizGenerationAgent(BaseAIAgent):
    ...
    def design_quiz_structure(
        ...
        try:
            ...
            logger.info(f"Designed quiz structure: {structure.purpose}, {structure.total_questions} questions")
            return structure
    ...
    def execute_task(self, context: GenerationContext, decision: AgentDecision) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            enhanced_questions = self._enhance_question_quality(all_questions, context, strategy)
            ...
            logger.info(f"Generated {len(question_dicts)} quiz questions using agentic approach")
    ...
    def _create_default_sections(self, structure: QuizStructure) -> List[Dict[str, Any]]:
        ...
        remaining = structure.total_questions - sum(s["question_count"] for s in sections)
        if remaining > 0:
            sections.append({
                "name": "challenge",
    ...
    def _apply_quality_improvements(self, question: QuizQuestion, context: GenerationContext) -> QuizQuestion:
        """Apply specific quality improvements to a question."""
        # Ensure question clarity
        if len(question.question_text) < 10:
            question.question_text = f"Regarding {context.topic}: {question.question_text}"
        
        # Ensure adequate explanation
        if not question.explanation and question.question_type != "essay":
            question.explanation = f"This question tests understanding of {context.topic} concepts."
        ...
        if question.question_type == "multiple_choice":
            if len(question.answer_options) < 3:
                ...
                while len(question.answer_options) < 4:
                    question.answer_options.append(f"Option {len(question.answer_options) + 1}")
    ...
    def _sequence_questions(
        ...
        if strategy.question_sequencing == "difficulty_progression":
            ...
            return sorted(questions, key=lambda q: difficulty_order.get(q.difficulty_level, 2))
            ...
            return sorted(questions, key=lambda q: cognitive_order.get(q.cognitive_domain, 3))
            ...
            other_questions = [q for q in questions if q.question_type != "multiple_choice"]
