...
class FlashcardItem(BaseModel):
    ...
    explanation: str = Field(
        description="Additional explanation or context", default=""
    )
...
class FlashcardGenerationPlan(BaseModel):
    ...
    format_distribution: dict[str, int] = Field(
        description="Distribution of card formats"
    )
...
class FlashcardGenerationAgent(BaseAIAgent):
    ...
    def execute_task(
        ...
        try:
            ...
            logger.info(
                f"Generated {len(flashcard_dicts)} flashcards using agentic approach"
            )
    ...
    def _get_batch_focus(
        self, plan: FlashcardGenerationPlan, batch_number: int, count: int
    ) -> dict[str, Any]:
    ...
    def _enhance_batch_quality(
        self, batch: FlashcardBatch, context: GenerationContext
    ) -> FlashcardBatch:
        ...
        try:
            ...
            enhancement_prompt = ChatPromptTemplate.from_messages(
                [
                    (
                        "system",
                        """You are a quality enhancement specialist for educational flashcards.
