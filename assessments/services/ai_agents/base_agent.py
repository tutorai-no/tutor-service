...
class AgentDecision(BaseModel):
    ...
    parameters: Dict[str, Any] = Field(description="Parameters for the action", default={})
    fallback_actions: List[str] = Field(description="Alternative actions if this fails", default=[])
...
class ContentAnalysis(BaseModel):
    ...
    concept_relationships: List[Tuple[str, str]] = Field(description="Relationships between concepts", default=[])
    suitable_formats: List[str] = Field(description="Suitable question/card formats")
    estimated_items: int = Field(description="Estimated number of items that can be generated")
    quality_indicators: Dict[str, float] = Field(description="Quality metrics", default={})
...
class BaseAIAgent(ABC):
    ...
    def __init__(self, role: AgentRole, model_name: str = None, temperature: float = 0.2):
        """
    ...
    def make_decision(self, context: GenerationContext, analysis: ContentAnalysis) -> AgentDecision:
        """
    ...
    def reflect_on_results(self, generated_items: List[Dict[str, Any]], context: GenerationContext) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            result = self.llm.invoke(reflection_prompt.format(
                role=self.role.value,
    ...
    def enhance_with_retrieval(self, context: GenerationContext, query: str) -> str:
        ...
        try:
            ...
            if enhanced_content:
                logger.info(f"Enhanced content with {len(enhanced_content)} characters from retrieval")
        ...
        try:
            ...
            if primary_agent_role in self.agents:
                ...
                if use_quality_assessment and AgentRole.QUALITY_ASSESSOR in self.agents:
                    ...
                    quality_report = assessor.reflect_on_results(generated_content, context)
                    ...
                    workflow_results["agents_used"].append(AgentRole.QUALITY_ASSESSOR.value)
                
            else:
                raise ValueError(f"Primary agent {primary_agent_role.value} not registered")
