...
class QuizGenerationService:
    ...
    def generate_quiz(
        ...
        try:
            ...
            logger.info(
                f"Successfully generated quiz with {len(generated_questions)} questions for course {course_id}"
            )
            return response
        ...
        try:
            with transaction.atomic():
                ...
                quiz = Quiz.objects.create(
                    ...
                    time_limit_minutes=context.constraints.get(
                ...
                logger.info(
                    f"Saved quiz '{title}' with {len(saved_questions)} questions to database"
                )
                return quiz_dict
        ...
        try:
            ...
            if result.get("success"):
                ...
                result["adaptive_metadata"] = {
                    ...
                    "user_performance_level": (
                        performance_history.get("average_score", 0.0)
                        if performance_history
                        else 0.0
                    ),
                    "adaptation_strategy": "difficulty_based",
                    "recommended_next_topics": self._get_next_topic_recommendations(
                        topic, start_difficulty
                    ),
                }
    ...
    def _get_next_topic_recommendations(
        self, current_topic: str, difficulty: str
    ) -> list[str]:
    ...
    def get_quiz_recommendations(
        ...
        try:
            ...
            return {
                ...
                "user_analysis": {
                    "performance_level": (
                        user_performance.get("average_score", 0.0)
                        if user_performance
                        else None
                    ),
