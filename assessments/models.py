...
class Flashcard(models.Model):
    ...
    explanation = models.TextField(blank=True, null=True, help_text="Additional explanation")
    ...
    source_content = models.TextField(blank=True, null=True, help_text="Source text used to generate this flashcard")
    ...
    interval_days = models.PositiveIntegerField(
        default=1,
        help_text="Current interval in days"
    )
    repetitions = models.PositiveIntegerField(
        default=0,
        help_text="Number of successful repetitions"
    )
...
class FlashcardReview(models.Model):
    ...
    quality_response = models.PositiveSmallIntegerField(
        ...
        help_text="Quality of recall (0-5)"
    )
...
class QuizAttempt(models.Model):
    ...
    questions_order = models.JSONField(
        default=list,
        help_text="Order of questions for this attempt"
    )
    ...
    def __str__(self):
        return f"Attempt {self.attempt_number}: {self.quiz.title} by {self.user.username}"
...
class Assessment(models.Model):
    ...
    def calculate_performance(self):
        ...
        flashcard_reviews = FlashcardReview.objects.filter(
            flashcard__course=self.course,
            user=self.user
        )
        
        if flashcard_reviews.exists():
            flashcard_performance = flashcard_reviews.aggregate(
                avg_quality=models.Avg('quality_response')
            )['avg_quality'] / 5.0  # Normalize to 0-1
        ...
        if quiz_attempts.exists():
            quiz_performance = quiz_attempts.aggregate(
                avg_score=models.Avg('percentage_score')
            )['avg_score'] / 100.0  # Normalize to 0-1
...
class StudyStreak(models.Model):
    ...
    def __str__(self):
        ...
        return f"{self.user.username} - {self.get_streak_type_display()} - {course_name}"
    ...
    def check_milestones(self):
        ...
        for level in milestone_levels:
            if (self.current_streak >= level and 
                level not in self.milestones_achieved):
