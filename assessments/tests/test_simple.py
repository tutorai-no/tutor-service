...
from assessments.tests.fixtures import CourseFactory, FlashcardFactory, UserFactory
...
class TestDataModels(TestCase):
    ...
    def test_generation_context_creation(self):
        ...
        context = GenerationContext(
            course_id=1, user_id=1, content="Test content", topic="Test Topic"
        )
...
class TestSpacedRepetitionService(TestCase):
    ...
    def test_calculate_next_review_correct_response(self):
        ...
        ease_factor, interval, repetitions, next_date = (
    ...
    def test_difficulty_multiplier(self):
        ...
        unknown_multiplier = SpacedRepetitionService.get_difficulty_multiplier(
            "unknown"
        )
    ...
    def test_adjust_for_difficulty(self):
        ...
        easy_interval = SpacedRepetitionService.adjust_for_difficulty(
            base_interval, "easy"
        )
        hard_interval = SpacedRepetitionService.adjust_for_difficulty(
            base_interval, "hard"
        )
    ...
    def test_get_optimal_batch_size(self):
        ...
        batch_size = SpacedRepetitionService.get_optimal_batch_size(
            total_due=100, available_time_minutes=30, avg_time_per_card=30  # 30 seconds
        )
        self.assertEqual(batch_size, 50)  # min(100, 60, 50) = 50

        # Test time-constrained case
        batch_size = SpacedRepetitionService.get_optimal_batch_size(
            total_due=100, available_time_minutes=10, avg_time_per_card=30
        )
        self.assertEqual(batch_size, 20)  # 10 * 60 / 30 = 20

        # Test minimum batch size
        batch_size = SpacedRepetitionService.get_optimal_batch_size(
            total_due=0, available_time_minutes=5, avg_time_per_card=60
        )
