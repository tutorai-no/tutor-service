...
class TestBaseAIAgent(BaseTestCase):
    ...
    def test_reflect_on_results(self):
        ...
        with patch.object(
            self.agent.llm, "invoke", return_value=Mock(content="Reflection response")
        ):
...
class TestContentGenerationOrchestrator(BaseTestCase):
    ...
    def test_get_agent_recommendations(self):
        ...
        simple_context = GenerationContext(
            course_id=1, user_id=1, content="Short content", topic="Simple topic"
        )
    ...
    def test_workflow_history_tracking(self):
        ...
        with patch.object(
            self.test_agent,
            "analyze_content",
            return_value=create_mock_content_analysis(),
        ):
            with patch.object(
                self.test_agent,
                "make_decision",
                ...
                with patch.object(
                    self.test_agent, "execute_task", return_value=[{"test": "result"}]
                ):
        ...
        self.assertEqual(
            len(self.orchestrator.workflow_history), initial_history_length + 1
        )
