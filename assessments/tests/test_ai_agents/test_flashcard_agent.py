...
from assessments.services.ai_agents.base_agent import AgentDecision, AgentRole
...
class TestFlashcardGenerationAgent(BaseTestCase):
    ...
    def test_create_generation_plan(self, mock_parser_class):
        ...
        with patch.object(
            self.agent.llm, "invoke", return_value=Mock(content="mock plan")
        ):
    ...
    def test_generate_flashcard_batch(self, mock_parser_class):
        ...
        with patch.object(
            self.agent.llm, "invoke", return_value=Mock(content="mock batch")
        ):
    ...
    def test_execute_task_success(self, mock_parser_class):
        ...
        with patch.object(
            self.agent.llm, "invoke", return_value=Mock(content="mock response")
        ):
    ...
    def test_execute_task_fallback(self):
        ...
        with patch.object(
            self.agent, "analyze_content", side_effect=Exception("Analysis failed")
        ):
            results = self.agent.execute_task(context, decision)
    ...
    def test_error_handling_in_plan_creation(self):
        ...
        with patch.object(
            self.agent.llm, "invoke", side_effect=Exception("Plan creation failed")
        ):
            plan = self.agent.create_generation_plan(context, analysis)
