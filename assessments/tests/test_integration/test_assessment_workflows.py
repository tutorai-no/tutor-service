...
from assessments.models import Flashcard, Quiz, QuizQuestion
...
from assessments.tests.fixtures import AssessmentFactory, CourseFactory, UserFactory
...
class TestEndToEndFlashcardWorkflow(TransactionTestCase):
    ...
    def setUp(self):
        ...
        mock_retrieval_client.get_context.return_value = (
            "Mock educational content about machine learning algorithms"
        )
    ...
    def test_flashcard_generation_error_recovery(self):
        ...
        result = service.generate_flashcards(
            user_id=999999, course_id=self.course.id, topic="Test Topic"
        )
        ...
        result = service.generate_flashcards(
            user_id=self.user.id, course_id=999999, topic="Test Topic"
        )
...
class TestAssessmentAPIIntegration(APITestCase):
    ...
    @patch(
        "assessments.services.generators.flashcard_service.FlashcardGenerationService.generate_flashcards"
    )
    @patch(
        "assessments.services.generators.quiz_service.QuizGenerationService.generate_quiz"
    )
    ...
    @patch(
        "assessments.services.generators.quiz_service.QuizGenerationService.generate_adaptive_quiz"
    )
    def test_adaptive_quiz_generation_api(self, mock_adaptive_quiz):
        ...
        url = f"/api/v1/assessments/{self.assessment.id}/generate_content/"
        data = {"topic": "Data Structures", "use_adaptive": True}
...
class TestSpacedRepetitionIntegration(TransactionTestCase):
    ...
    def test_flashcard_review_updates_spaced_repetition(self):
        ...
        self.assertGreater(
            self.flashcard.ease_factor, 2.5
        )  # Should increase with perfect response
