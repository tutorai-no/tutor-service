...
class CustomUserAdmin(UserAdmin):
    ...
    list_display = [
        ...
        'university', 'study_level', 'is_verified', 
        'is_staff', 'is_active', 'created_at'
    ]
    list_filter = [
        'is_staff', 'is_active', 'is_verified', 'study_level', 
        'university', 'language', 'created_at'
    ]
    ...
    fieldsets = UserAdmin.fieldsets + (
        ('Profile Information', {
            'fields': ('phone_number', 'avatar', 'university', 'study_level', 
                      'study_year', 'field_of_study')
        }),
        ('Preferences', {
            'fields': ('timezone', 'language', 'preferred_study_time', 
                      'daily_study_goal_minutes')
        }),
        ('Marketing', {
            'fields': ('acquisition_source', 'acquisition_details')
        }),
        ('Status', {
    ...
    add_fieldsets = UserAdmin.add_fieldsets + (
        ('Profile Information', {
            'fields': ('email', 'first_name', 'last_name', 'university', 'study_level')
        }),
    )
...
class UserProfileAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Profile', {
            'fields': ('user', 'bio', 'learning_goals')
        }),
        ('Study Preferences', {
            'fields': ('study_style', 'difficulty_preference')
        }),
        ('Notification Settings', {
            'fields': ('email_notifications', 'study_reminders', 
...
class UserActivityAdmin(admin.ModelAdmin):
    list_display = ['user', 'activity_type', 'resource_type', 'device_type', 'created_at']
    ...
    fieldsets = (
        ('Activity', {
            'fields': ('user', 'activity_type', 'session_id')
        }),
        ('Resource Context', {
            'fields': ('resource_type', 'resource_id')
        }),
        ('Performance', {
            'fields': ('duration_seconds', 'metadata')
        }),
        ('Device Information', {
...
class UserStreakAdmin(admin.ModelAdmin):
    list_display = [
        ...
        'last_activity_date', 'total_study_sessions'
    ]
    ...
    fieldsets = (
        ...
        ('Timing', {
            'fields': ('current_streak_start', 'last_activity_date')
        }),
        ('Statistics', {
            'fields': ('total_study_days', 'total_study_sessions')
        }),
        ('Milestones', {
            'fields': ('streak_milestones_achieved',)
        }),
        ('Timestamps', {
            'fields': ('created_at', 'updated_at'),
            'classes': ('collapse',)
...
class UserFeedbackAdmin(admin.ModelAdmin):
    list_display = [
        'title', 'user', 'feedback_type', 'status', 'priority', 
        'satisfaction_rating', 'created_at'
    ]
    list_filter = ['feedback_type', 'status', 'priority', 'satisfaction_rating', 'created_at']
    ...
    fieldsets = (
        ('Feedback', {
            'fields': ('user', 'feedback_type', 'title', 'description')
        }),
        ('Attachments', {
            'fields': ('screenshot',)
        }),
...
class UserApplicationAdmin(admin.ModelAdmin):
    list_display = [
        ...
        'acquisition_source', 'status', 'created_at'
    ]
    ...
    fieldsets = (
        ...
        ('Background', {
            'fields': ('university', 'study_level', 'field_of_study')
        }),
        ('Acquisition', {
