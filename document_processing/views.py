...
class CourseHierarchyView(APIView):
    ...
    def get(self, request, course_id):
        ...
        try:
            hierarchy_data = service.knowledge_graph_service.get_course_hierarchy_from_neo4j(course_id)
            ...
            return Response({
                ...
                'stats': {
                    ...
                    'main_topics': len([n for n in hierarchy_data.get('nodes', []) if n.get('type') == 'MAIN_TOPIC']),
                    'subtopics': len([n for n in hierarchy_data.get('nodes', []) if n.get('type') == 'SUBTOPIC'])
                }
            })
...
class CourseTopicsView(APIView):
    ...
    def get(self, request, course_id):
        ...
        try:
            ...
            return Response({
                'course_id': course_id,
                'course_name': course_node.get('title', 'Unknown Course') if course_node else 'Unknown Course',
        ...
        try:
            ...
            for edge in edges:
                if edge.get('from') == topic_id:
                    ...
                    child_node = next((n for n in nodes if n.get('id') == edge.get('to')), None)
                    if child_node:
                        children.append(child_node)
                elif edge.get('to') == topic_id:
                    # This topic is child of another
                    parent_node = next((n for n in nodes if n.get('id') == edge.get('from')), None)
