...
if LANGCHAIN_AVAILABLE:
    class Topic(BaseModel):
        ...
        level: int = Field(description="Hierarchy level (1=main topic, 2=subtopic, etc.)")
        parent_topic: Optional[str] = Field(None, description="Parent topic title for subtopics")
        ...
        page_references: List[int] = Field(default_factory=list, description="Page numbers where topic appears")
    ...
    class HierarchicalTopicStructure(BaseModel):
        ...
        subtopics: List[Topic] = Field(default_factory=list, description="List of subtopics (level 2+)")
    ...
    def _setup_llm(self):
        ...
        try:
            ...
            if not openai_api_key:
                logger.warning("OPENAI_API_KEY not configured, topic extraction disabled")
                return
            ...
            # Set up parser
            self.parser = PydanticOutputParser(pydantic_object=HierarchicalTopicStructure)
...
"""
    ...
    def extract_topics_from_toc(self, text: str) -> Dict[str, Any]:
        ...
        toc_indicators = [
            ...
            'tabla de contenidos', 'índice', 'sommaire'
        ]
        ...
        try:
            ...
            for topic in result.subtopics:
                topic_dict = {
                    ...
                    'parent_id': self._generate_topic_id(topic.parent_topic) if topic.parent_topic else None,
                    'description': topic.description,
                    'keywords': topic.keywords,
                    'page_references': topic.page_references
                }
            ...
            logger.info(f"Extracted {len(topic_data['main_topics'])} main topics and {len(topic_data['subtopics'])} subtopics")
        ...
        for subtopic in topic_data['subtopics']:
            subtopic_node = {
                ...
                'properties': {
                    ...
                    'page_refs': ','.join(map(str, subtopic.get('page_references', [])))
                }
            }
