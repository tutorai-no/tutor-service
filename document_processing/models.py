...
class DocumentUpload(models.Model):
    ...
    course = models.ForeignKey('courses.Course', on_delete=models.CASCADE, related_name='document_uploads', null=True, blank=True)
...
class DocumentChunk(models.Model):
    ...
    # Metadata
    page_numbers = models.JSONField(default=list, blank=True)  # Which pages this chunk spans
    metadata = models.JSONField(default=dict, blank=True)  # Additional metadata from extraction
...
class URLUpload(models.Model):
    ...
    course = models.ForeignKey('courses.Course', on_delete=models.CASCADE, related_name='url_uploads', null=True, blank=True)
...
class URLChunk(models.Model):
    ...
    """
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    url_upload = models.ForeignKey(URLUpload, on_delete=models.CASCADE, related_name='chunks')
    ...
    # Metadata
    metadata = models.JSONField(default=dict, blank=True)  # Additional metadata from scraping
