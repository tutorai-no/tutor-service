...
class StudyPlan(BaseModel):
    ...
    course = models.ForeignKey(
        "courses.Course", on_delete=models.CASCADE, related_name="study_plans"
    )
    ...
    plan_data = models.JSONField(
        default=dict, help_text="AI-generated plan structure with tasks and schedules"
    )
...
class StudySession(BaseModel):
    ...
    topics = models.JSONField(
        default=list, help_text="List of topics to cover in this session"
    )
    materials = models.JSONField(
        default=list, help_text="List of materials/documents for this session"
    )
...
class LearningProgress(BaseModel):
    ...
    course = models.ForeignKey(
        "courses.Course", on_delete=models.CASCADE, related_name="learning_progress"
    )

    progress_type = models.CharField(max_length=20, choices=PROGRESS_TYPES)
    identifier = models.CharField(
        max_length=200, help_text="Topic name, skill identifier, etc."
    )
    ...
    metadata = models.JSONField(
        default=dict, help_text="Additional progress data and analytics"
    )
    ...
    def __str__(self):
        return (
            f"{self.user.username} - {self.identifier} ({self.completion_percentage}%)"
        )

    def update_progress(
        self, completion_delta=0, time_spent=None, performance_data=None
    ):
        """Update progress metrics."""
        if completion_delta:
            self.completion_percentage = min(
                100, self.completion_percentage + completion_delta
            )
...
class StudyGoal(BaseModel):
    ...
    study_plan = models.ForeignKey(
        StudyPlan, on_delete=models.CASCADE, related_name="goals", null=True, blank=True
    )
    ...
    ai_rationale = models.TextField(
        blank=True, help_text="AI explanation for suggested goal"
    )
...
class StudyRecommendation(BaseModel):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="study_recommendations"
    )
    course = models.ForeignKey(
        "courses.Course",
        on_delete=models.CASCADE,
    ...
    recommendation_data = models.JSONField(
        default=dict, help_text="Structured data for the recommendation"
    )
