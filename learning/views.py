...
class StudyPlanViewSet(viewsets.ModelViewSet):
    ...
    def activate(self, request, pk=None):
        ...
        StudyPlan.objects.filter(
            user=request.user, course=study_plan.course, is_active=True
    ...
    def performance_analysis(self, request, pk=None):
        ...
        analysis = performance_service.analyze_comprehensive_performance(
            user=request.user, course=study_plan.course, time_period_days=time_period
        )
    ...
    def _generate_optimal_schedule(
        self, study_plan, hours_per_day, preferred_times, include_weekends
    ):
        ...
        while current_date <= end_date and goals:
            if include_weekends or current_date.weekday() < 5:  # Monday = 0, Sunday = 6
                ...
                for goal in goals:
                    ...
                    session_hours = min(
                        remaining_hours, goal.estimated_hours / 10
                    )  # Spread over days
                    if session_hours > 0.25:  # Minimum 15 minutes
                        session = {
                            ...
                            "focus_areas": (
                                goal.key_concepts[:3] if goal.key_concepts else []
                            ),
                        }
    ...
    def _generate_recommendations(self, study_plan, user):
            ...
            recommendations.append(
                {
                    ...
                    "metadata": {
                        ...
                        "focus_goals": [
                            str(g.id)
                            for g in study_plan.goals.filter(status="in_progress")[:3]
                        ],
                    },
                }
            )
        ...
        low_performance_goals = study_plan.goals.filter(
            progress_percentage__lt=50, status="in_progress"
        )
            ...
            recommendations.append(
                {
                    ...
                    "metadata": {
                        "struggling_goals": [
                            str(g.id) for g in low_performance_goals[:3]
                        ],
                        "suggested_resources": [
                            "practice_problems",
                            "video_tutorials",
        ...
        for session in sessions:
            ...
            if last_date is None:
                ...
                if session_date == timezone.now().date() or (
                    ...
                    and (last_date - session_date).days == 1
                ):
    ...
    def perform_create(self, serializer):
        ...
        if study_plan.user != self.request.user:
            raise permissions.PermissionDenied(
                "You can only create goals for your own study plans"
            )
        serializer.save()
...
class LearningProgressViewSet(viewsets.ModelViewSet):
    ...
    def summary(self, request):
        ...
        summary = {
            ...
            "by_type": entries.values("progress_type")
            .annotate(count=Count("id"))
            .order_by("-count"),
            "goals_completed": entries.filter(progress_type="goal_completed").count(),
            "study_sessions": entries.filter(progress_type="study_session").count(),
            "milestones_reached": entries.filter(progress_type="milestone").count(),
        }
...
class LearningAnalyticsViewSet(viewsets.ViewSet):
    ...
    def overview(self, request):
        ...
        analytics = {
            ...
            "completion_rate": round(
                (completed_goals / goals.count() * 100) if goals.count() > 0 else 0, 1
            ),
    ...
    def performance_trends(self, request):
        ...
        goal_trends = (
            ...
            .annotate(
                completed=Count("id"),
                avg_time_to_complete=Avg(F("actual_completion_date") - F("created_at")),
            )
            .order_by("month")
        )
    ...
    def adaptive_performance_analysis(self, request):
        ...
        performance_service = get_performance_analysis_service()
        analysis = performance_service.analyze_comprehensive_performance(
            user=request.user, course=course, time_period_days=time_period
        )
    ...
    def performance_trajectory(self, request):
        ...
        trajectory = performance_service.predict_performance_trajectory(
            user=request.user, course=course, prediction_days=prediction_days
        )
    ...
    def completion_predictions(self, request):
        ...
        # Get active study plans
        active_plans = StudyPlan.objects.filter(user=request.user, is_active=True)
        ...
        for plan in active_plans:
            prediction = prediction_service.predict_course_completion(
                user=request.user, course=plan.course
            )
    ...
    def _calculate_learning_velocity(self, user, start_date):
        ...
        return {
            ...
            "velocity_ratio": round(
                (
                    goals_per_week / target_goals_per_week
                    if target_goals_per_week > 0
                    else 0
                ),
                2,
            ),
            "status": (
                "on_track"
                if goals_per_week >= target_goals_per_week * 0.9
                else "behind"
            ),
        }
    ...
    def _generate_analytics_recommendations(
        self, user, study_hours, goals_completed, patterns
    ):
    ...
    def _calculate_time_estimates(self, study_plan):
        ...
        return {
            ...
            "weeks_to_complete": (
                round(weeks_to_complete, 1) if weeks_to_complete else None
            ),
            "estimated_completion_date": (
                estimated_completion.date().isoformat()
    ...
    def _calculate_success_probability(self, study_plan):
        ...
        if time_ratio > 0:
            progress_ratio = (
                progress["completion_percentage"] / progress["expected_completion"]
            )
        ...
        return {
            ...
            "confidence": (
                "high" if probability > 80 else "medium" if probability > 50 else "low"
            ),
            "factors": {
...
class AdaptiveLearningViewSet(viewsets.ViewSet):
    ...
    def generate_adaptive_plan(self, request):
        ...
        if result["success"]:
            ...
            study_plan = StudyPlan.objects.create(
                ...
                study_days_per_week=(
                    5 if not preferences.get("include_weekends", True) else 7
                ),
                status="active",
            )
    ...
    def process_study_session(self, request):
        ...
        for topic in session_data.get("topics_covered", []):
            performance_data = {
                ...
                "response_time": session_data.get("duration_minutes", 30)
                * 60
                / max(1, len(session_data.get("topics_covered", [topic]))),
                "difficulty": "medium",
        ...
        if active_plan:
            ...
            adaptation_result = generator.adapt_existing_plan(
                active_plan, performance_update=feedback
            )

            if adaptation_result.get("success") and adaptation_result.get(
                "adaptations_made"
            ):
        ...
        return Response(
            {
                ...
                "recommendations": (
                    adaptation_result.get("recommendations", [])
    ...
    def learning_insights(self, request):
        ...
        trajectory = performance_service.predict_performance_trajectory(
            user=request.user, course=course, prediction_days=30
        )

        # Get review schedule optimization
        review_service = get_review_scheduling_service()
        review_optimization = review_service.optimize_daily_review_load(
            user=request.user, target_daily_reviews=50, max_daily_reviews=100
        )
        ...
        return Response(
            {
                ...
                "learning_summary": {
                    ...
                    "performance_category": performance_analysis.get(
                        "performance_category", "unknown"
                    ),
                    "main_strengths": performance_analysis.get(
                        "strengths_weaknesses", {}
                    ).get("strengths", []),
                    "improvement_areas": performance_analysis.get(
                        "strengths_weaknesses", {}
                    ).get("weaknesses", []),
                    "key_recommendations": performance_analysis.get(
                        "recommendations", []
                    )[:3],
                },
            }
    ...
    def manual_override(self, request):
        ...
        override_type = request.data.get(
            "override_type"
        )  # 'schedule', 'difficulty', 'review_frequency'
        ...
        # Get study plan
        try:
            study_plan = StudyPlan.objects.get(id=study_plan_id, user=request.user)
        except StudyPlan.DoesNotExist:
            return Response(
                {"error": "Study plan not found"}, status=status.HTTP_404_NOT_FOUND
            )
    ...
    def adaptive_dashboard(self, request):
        ...
        overall_analysis = performance_service.analyze_comprehensive_performance(
            user=request.user, course=None, time_period_days=30  # All courses
        )

        # Get active study plans with predictions
        active_plans = StudyPlan.objects.filter(user=request.user, status="active")
        ...
        dashboard_data = {
            "overall_performance": {
                ...
                "trend": overall_analysis.get("trends", {}).get(
            ...
            "review_workload": {
                "today": review_summary.get("load_metrics", {}).get(
                    "average_daily_reviews", 0
                ),
                "this_week": review_summary.get("load_metrics", {}).get(
                    "peak_load_day", 0
                ),
                "optimization_needed": len(review_summary.get("recommendations", [])),
            },
            "key_strengths": overall_analysis.get("strengths_weaknesses", {}).get(
                "strengths", []
            )[:3],
            "improvement_areas": overall_analysis.get("strengths_weaknesses", {}).get(
                "weaknesses", []
            )[:3],
