...
class AdaptiveLearningService(ABC):
    ...
    def log_adaptation(self, user_id: str, adaptation_type: str, details: Dict[str, Any]) -> None:
        """
...
class TimeSlotOptimizer:
    ...
    def get_optimal_time_slots(
        ...
        # Generate time slots
        time_slots = []
        preferred_hours = self.preferences.get('preferred_hours', optimal_times['optimal_hours'])
        ...
        for day in days_available:
            for i in range(sessions_per_day):
                ...
                hour = preferred_hours[i % len(preferred_hours)] if preferred_hours else 9
                
                time_slots.append({
                    'day': day,
                    'start_time': time(hour, 0),
                    'duration_minutes': session_length,
                    'productivity_score': optimal_times['hourly_productivity'].get(hour, 3.0)
                })
