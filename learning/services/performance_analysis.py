...
from django.db.models import QuerySet, Avg, Count, Q, Sum, Max, Min
...
class PerformanceAnalysisService(AdaptiveLearningService):
    ...
    def analyze_comprehensive_performance(
        ...
        try:
            ...
            # Initialize metrics
            metrics = StudyMetrics(user, course, time_period_days)
            
            # Perform various analyses
            quiz_analysis = self._analyze_quiz_performance(user, course, time_period_days)
            session_analysis = self._analyze_study_sessions(user, course, time_period_days)
            progress_analysis = self._analyze_learning_progress(user, course, time_period_days)
            flashcard_analysis = self._analyze_flashcard_performance(user, course, time_period_days)
            engagement_analysis = self._analyze_engagement_patterns(user, course, time_period_days)
            ...
            recommendations = self._generate_adaptive_recommendations(
                overall_score, strengths_weaknesses, {
                    'quiz': quiz_analysis,
                    'sessions': session_analysis,
                    'progress': progress_analysis,
                    'flashcards': flashcard_analysis,
            ...
            return {
                ...
                'analysis_period': {
                    ...
                    'start_date': (timezone.now() - timedelta(days=time_period_days)).date().isoformat(),
    ...
    def _analyze_quiz_performance(self, user, course, time_period_days: int) -> Dict[str, Any]:
        """Analyze quiz performance in detail."""
        from assessments.models import Quiz, QuizAttempt
        
        attempts = QuizAttempt.objects.filter(
            user=user,
            created_at__gte=timezone.now() - timedelta(days=time_period_days)
        )
    ...
    def _analyze_study_sessions(self, user, course, time_period_days: int) -> Dict[str, Any]:
        ...
        productivity_scores = list(sessions.filter(
            productivity_rating__isnull=False
        ).values_list('productivity_rating', flat=True))
    ...
    def _analyze_learning_progress(self, user, course, time_period_days: int) -> Dict[str, Any]:
        ...
        progress_entries = LearningProgress.objects.filter(
            user=user,
            updated_at__gte=timezone.now() - timedelta(days=time_period_days)
        )
        ...
        return {
            ...
            'topics_in_progress': progress_entries.filter(mastery_level__in=[2, 3]).count(),
    ...
    def _analyze_engagement_patterns(self, user, course, time_period_days: int) -> Dict[str, Any]:
    ...
    def _calculate_overall_performance_score(self, analyses: Dict[str, Dict[str, Any]]) -> float:
        ...
        scores['progress'] = (analyses['progress'].get('average_mastery_level', 0) / 5) * 100
    ...
    def _identify_strengths_and_weaknesses(self, analyses: Dict[str, Dict[str, Any]]) -> Dict[str, List[str]]:
    ...
    def _analyze_performance_trends(self, user, course, time_period_days: int) -> Dict[str, Any]:
    ...
    def _calculate_time_efficiency(self, quiz_attempts) -> float:
        ...
        total_questions = sum(
            attempt.quiz.total_questions for attempt in attempts_with_time
    ...
    def _analyze_session_timing(self, sessions) -> Dict[str, Any]:
        ...
        # Find best time
        best_hour = max(hourly_averages.items(), key=lambda x: x[1])[0] if hourly_averages else 9
    ...
    def _calculate_improvement_rate(self, progress_entries) -> float:
        ...
        weekly_progress = progress_entries.annotate(
    ...
    def _count_mastered_cards(self, reviews) -> int:
        ...
        card_reviews = reviews.values('flashcard').annotate(
            last_reviews=Count('id')
    ...
    def _calculate_engagement_score(self, sessions, study_streak: int, time_period_days: int) -> float:
        """Calculate overall engagement score."""
        # Factors: session frequency, streak, completion rate
        session_frequency = sessions.count() / time_period_days
        completion_rate = sessions.filter(session_completed=True).count() / max(1, sessions.count())
    ...
    def _calculate_consistency_score(self, daily_activity) -> float:
        ...
        variance = sum((x - mean_activity) ** 2 for x in activity_counts) / len(activity_counts)
    ...
    def _get_quiz_score_trends(self, user, course, time_period_days: int) -> Dict[str, Any]:
        ...
        # Group by week
        weekly_scores = attempts.annotate(
    ...
    def _get_study_time_trends(self, user, course, time_period_days: int) -> Dict[str, Any]:
    ...
    def _get_engagement_trends(self, user, course, time_period_days: int) -> Dict[str, Any]:
        ...
        weekly_engagement = sessions.annotate(
            week=TruncWeek('created_at')
        ).values('week').annotate(
            session_count=Count('id'),
        ...
        return {
            ...
            'engagement_change': engagement_scores[-1] - engagement_scores[0] if len(engagement_scores) > 1 else 0
        }
    ...
    def _analyze_quiz_completion(self, user, recent_activity: Dict[str, Any]) -> Dict[str, Any]:
        """Analyze recent quiz completion for real-time feedback."""
        quiz_score = recent_activity.get('score', 0)
        time_taken = recent_activity.get('time_taken', 0)
        ...
        if score_diff < -10:
            feedback['immediate_recommendations'].append('Review the topics covered in this quiz')
            feedback['immediate_recommendations'].append('Consider additional practice before the next quiz')
        elif score_diff > 10:
            feedback['immediate_recommendations'].append('Great improvement! Consider tackling more challenging content')
        
        return feedback
    
    def _analyze_study_session_completion(self, user, recent_activity: Dict[str, Any]) -> Dict[str, Any]:
        ...
        feedback = {
            'session_quality': 'good' if productivity >= 3 and completed else 'needs_improvement',
            'duration_feedback': self._evaluate_session_duration(duration),
            'immediate_recommendations': []
        }
        
        if not completed:
            feedback['immediate_recommendations'].append('Try shorter sessions to improve completion rate')
        
        if productivity < 3:
            feedback['immediate_recommendations'].append('Consider studying during your peak productivity hours')
            feedback['immediate_recommendations'].append('Eliminate distractions for better focus')
        
        return feedback
    
    def _analyze_flashcard_session(self, user, recent_activity: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if correct_rate < 0.6:
            feedback['immediate_recommendations'].append('Increase review frequency for these cards')
            feedback['immediate_recommendations'].append('Try creating memory associations')
    ...
    def _calculate_performance_trajectory(self, trends: Dict[str, Any], prediction_days: int) -> Dict[str, Any]:
        ...
        if trajectory_direction == 'improving':
            trajectory_strength = 'strong' if current_trends.count('improving') >= 2 else 'moderate'
        elif trajectory_direction == 'declining':
            trajectory_strength = 'concerning' if current_trends.count('declining') >= 2 else 'moderate'
        else:
    ...
    def _assess_performance_risks(self, user, course, trends: Dict[str, Any]) -> Dict[str, List[str]]:
    ...
    def _recommend_interventions(self, trajectory: Dict[str, Any], risks: Dict[str, List[str]]) -> List[Dict[str, Any]]:
