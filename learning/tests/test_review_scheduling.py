...
class TestReviewSchedulingService(TestCase):
    ...
    def test_schedule_intelligent_reviews_success(self):
        ...
        result = self.service.schedule_intelligent_reviews(
            user=self.user, course=self.course, target_retention=0.85
        )
    ...
    def test_get_concepts_for_review(self):
        ...
        QuizAttempt.objects.create(
            ...
            quiz=quiz,
            score=65,
            status="completed",  # Below 75% threshold
        )
    ...
    def test_optimize_daily_review_load(self):
        ...
        result = self.service.optimize_daily_review_load(
            user=self.user, target_daily_reviews=50, max_daily_reviews=100
        )
    ...
    def test_calculate_flashcard_next_review_sm2(self):
        ...
        performance_data = {"difficulty": "easy", "response_time": 2, "correct": True}
        ...
        for difficulty in ["easy", "medium", "hard", "again"]:
            next_review = self.service._calculate_flashcard_next_review(
                str(self.flashcard.id), difficulty, 5, True, self.user
            )
            self.assertIsInstance(next_review, datetime)

    def test_calculate_topic_next_review(self):
        """Test topic review date calculation."""
        performance = {"mastery_level": 3, "understanding_score": 75}

        next_review = self.service._calculate_topic_next_review(
            str(self.progress.id), performance, self.user
        )
    ...
    def test_update_item_difficulty(self):
        ...
        new_difficulty = self.service._update_item_difficulty(
            str(self.flashcard.id), "flashcard", performance
        )
    ...
    def test_calculate_optimal_review_date(self):
        ...
        item = {"difficulty": "medium", "type": "flashcard"}

        retention_analysis = {"average_retention": 0.75}
    ...
    def test_mastery_to_difficulty_conversion(self):
        ...
        test_cases = [(1, "hard"), (2, "hard"), (3, "medium"), (4, "easy"), (5, "easy")]
    ...
    def test_calculate_topic_priority(self):
        ...
        high_priority_progress = LearningProgress.objects.create(
            user=self.user, course=self.course, topic="Hard Topic", mastery_level=1
        )
        ...
        medium_priority_progress = LearningProgress.objects.create(
            user=self.user, course=self.course, topic="Medium Topic", mastery_level=3
        )
    ...
    def test_analyze_difficulty_patterns(self):
        ...
        # Convert to queryset
        reviews_qs = FlashcardReview.objects.filter(id__in=[r.id for r in reviews])
    ...
    def test_error_handling(self, mock_logger):
        ...
        result = self.service.schedule_intelligent_reviews(
            user=None, course=self.course, target_retention=0.85
        )
    ...
    def test_performance_impact_analysis(self):
        ...
        good_performance = {"correct": True, "response_time": 3, "confidence": 5}
        ...
        poor_performance = {"correct": False, "response_time": 15, "confidence": 1}
    ...
    def test_scheduling_confidence_calculation(self):
        ...
        retention_analysis = {"forgetting_curve": {"model_confidence": 0.8}}
    ...
    def test_different_target_retention_rates(self):
        ...
        for target_retention in [0.7, 0.8, 0.85, 0.9, 0.95]:
            result = self.service.schedule_intelligent_reviews(
                user=self.user, course=self.course, target_retention=target_retention
            )

            self.assertTrue(
                result["success"], f"Failed for retention rate {target_retention}"
            )
            self.assertEqual(
                result["scheduling_stats"]["target_retention"], target_retention
            )
