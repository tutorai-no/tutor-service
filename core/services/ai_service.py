...
class FlashcardData(BaseModel):
    ...
    difficulty_level: str = Field(
        description="Difficulty level: easy, medium, hard", default="medium"
    )
...
class QuizQuestionData(BaseModel):
    ...
    question_type: str = Field(
        description="Type: multiple_choice, short_answer, true_false"
    )
    answer_options: list[str] = Field(
        description="List of answer options for multiple choice", default=[]
    )
    correct_answers: list[str] = Field(description="List of correct answers")
    explanation: str = Field(
        description="Explanation of the correct answer", default=""
    )
    difficulty_level: str = Field(
        description="Difficulty level: easy, medium, hard", default="medium"
    )
...
class ContentGenerator:
    ...
    def generate_flashcards(
        self, content: str, num_cards: int = 10
    ) -> list[dict[str, str]]:
        """
    ...
    def generate_summary(self, content: str, max_length: int = 500) -> str:
        ...
        response = self.ai_service.generate_completion(
            prompt, max_tokens=max_length // 3
        )
    ...
    def generate_study_plan(
        self, course_content: str, duration_weeks: int = 4
    ) -> dict[str, Any]:
        """
