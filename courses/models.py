...
class Course(models.Model):
    ...
    course_code = models.CharField(
        max_length=50, blank=True, null=True, help_text="e.g., CS101, MATH201"
    )
    semester = models.CharField(
        max_length=50, blank=True, null=True, help_text="e.g., Fall 2024, Spring 2025"
    )
    academic_year = models.CharField(
        max_length=10, blank=True, null=True, help_text="e.g., 2024-2025"
    )
    ...
    color = models.CharField(
        max_length=7, default="#3B82F6", help_text="Hex color for UI representation"
    )
    ...
    preferred_study_methods = models.JSONField(
        default=list, help_text="Preferred learning tools (flashcards, quizzes, etc.)"
    )
...
class CourseSection(models.Model):
    ...
    learning_objectives = models.JSONField(
        default=list, help_text="List of learning objectives for this section"
    )
    key_concepts = models.JSONField(
        default=list, help_text="Key concepts covered in this section"
    )
    ...
    estimated_study_time = models.PositiveIntegerField(
        default=60, help_text="Estimated study time in minutes"
    )
...
class Document(models.Model):
    ...
    storage_path = models.CharField(
        max_length=1024, blank=True, help_text="Path in cloud storage"
    )
...
class DocumentTag(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="document_tags"
    )

    name = models.CharField(max_length=50)
    color = models.CharField(
        max_length=7, default="#6B7280", help_text="Hex color for tag display"
    )
    description = models.TextField(blank=True, null=True)
...
class DocumentTagAssignment(models.Model):
    ...
    tag = models.ForeignKey(
        DocumentTag, on_delete=models.CASCADE, related_name="document_assignments"
    )
