...
class DocumentViewSet(viewsets.ModelViewSet):
    ...
    def upload(self, request, course_pk=None):
        ...
        # Validate file size
        max_size = getattr(
            settings, "DOCUMENT_UPLOAD_MAX_SIZE", 50 * 1024 * 1024
        )  # 50MB default
        ...
        try:
            ...
            document = Document.objects.create(
                user=request.user, course=course, **document_data
            )

            # Coordinate with retrieval service for actual file processing
            upload_result = self._coordinate_file_upload(
                document, uploaded_file, request.user
            )
            ...
            if upload_result.get("success"):
                ...
                return Response(
                    {
                        ...
                        "estimated_processing_time": upload_result.get(
                ...
                document.processing_error = upload_result.get(
    ...
    def upload_url(self, request, course_pk=None):
        ...
        try:
            ...
            processing_result = self._coordinate_url_processing(
                document, source_url, request.user
            )
            ...
            if processing_result.get("success"):
                ...
                return Response(
                    {
                        ...
                        "estimated_processing_time": processing_result.get(
                ...
                # Processing failed
                document.processing_status = "failed"
                document.processing_error = processing_result.get(
    ...
    def processing_status(self, request, pk=None, course_pk=None):
        ...
        if status_result.get("success"):
            ...
            document.processing_status = status_data.get(
        ...
        return Response(
            {
                ...
                "estimated_completion_time": status_result.get(
    ...
    def reprocess(self, request, pk=None, course_pk=None):
        ...
        if reprocess_result.get("success"):
            return Response(
                {
                    ...
                    "estimated_time_minutes": reprocess_result.get(
                        "estimated_time_minutes", 5
                    ),
                }
            )
            ...
            document.processing_status = "failed"
            document.processing_error = reprocess_result.get(
    ...
    def content(self, request, pk=None, course_pk=None):
        ...
        if content_result.get("success"):
            ...
            return Response(
                {
                    "document_id": str(document.id),
                    "extracted_text": content_data.get(
                    ...
                    "embeddings_available": content_data.get(
    ...
    def _detect_document_type(self, uploaded_file):
        ...
        if content_type == "application/pdf" or filename.endswith(".pdf"):
            ...
            return "markdown"
        elif content_type in ["image/jpeg", "image/png"] or filename.endswith(
            (".jpg", ".jpeg", ".png")
        ):
            return "image"
        else:
    ...
    def _coordinate_file_upload(self, document, uploaded_file, user):
        ...
        try:
            ...
            if response.status_code != 200:
                logger.error(
                    f"Retrieval service upload failed: {response.status_code} - {response.text}"
                )
                return {
                    "success": False,
                ...
                for line in response.iter_lines(decode_unicode=True):
                    ...
                    try:
                        ...
                        if "chunk_index" in result and "text" in result:
                            ...
                            logger.debug(
                                f"Processed chunk {result.get('chunk_index')} for document {document.id}"
                            )
            ...
            if processing_started and chunks_processed > 0:
                logger.info(
                    f"Document {document.id} processing complete: {chunks_processed} chunks"
                )
                ...
                return {"success": False, "error": "No processing results received"}

        except requests.RequestException as e:
            logger.error(f"Error contacting retrieval service: {str(e)}")
            return {"success": False, "error": f"Service communication error: {str(e)}"}
    ...
    def _coordinate_url_processing(self, document, source_url, user):
        ...
        if not retrieval_service_url:
            logger.warning(
                "RETRIEVER_SERVICE_URL not configured, simulating processing"
            )
            return {
                "success": True,
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                logger.error(
                    f"URL processing failed: {response.status_code} - {response.text}"
                )
                return {
                    "success": False,
            ...
            logger.error(
                f"Error contacting retrieval service for URL processing: {str(e)}"
            )
            return {"success": False, "error": f"Service communication error: {str(e)}"}
    ...
    def _check_processing_status(self, document):
        ...
        try:
            ...
            response = requests.get(
                f"{retrieval_service_url}/api/v1/documents/{document.id}", timeout=10
            )
            ...
            if response.status_code == 200:
                ...
                if chunks_response.status_code == 200:
                    ...
                    if chunk_count > 0:
                        ...
                        return {
                            ...
                            "status": {"status": "processing", "completed": False},
                        }
                else:
                ...
                return {
                    ...
                    "status": {"status": "uploading", "completed": False},
                }
            else:
                return {
                    "success": False,
    ...
    def _request_reprocessing(self, document):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return {
                    "success": False,
                    "error": f"Reprocessing request failed: {response.status_code}",
                }
    ...
    def _get_document_content(self, document):
        ...
        try:
            ...
            if doc_response.status_code != 200:
                return {
                    "success": False,
                    "error": f"Document not found: {doc_response.status_code}",
                }
            ...
            if chunks_response.status_code != 200:
                return {
                    "success": False,
                    "error": f"Chunks retrieval failed: {chunks_response.status_code}",
                }
            ...
            return {
                ...
                "content": {
                    ...
                    "summary": summary,
                    "topics": (
                        key_concepts[:5] if key_concepts else ["processed", "content"]
                    ),
